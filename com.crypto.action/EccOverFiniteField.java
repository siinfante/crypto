...
public class EccOverFiniteField {
  ...
  public static void main(String[] args) throws Exception {
    ...
    System.out.println(
        "public key generation lasts "
            + (double) (generationEnd.getTime() - generationBegin.getTime()) / 1000
            + " seconds\n");
    ...
    System.out.println(
        "shared key exchange lasts "
            + (double) (exchangeEnd.getTime() - exchangeBegin.getTime()) / 1000
            + " seconds\n");
    ...
    BigInteger s =
        (hash.add(r.multiply(privateKey)).multiply(multiplicativeInverse(randomKey, order)))
            .remainder(order);
    ...
    System.out.println(
        "\nmessage signing lasts "
            + (double) (signingEnd.getTime() - signingBegin.getTime()) / 1000
            + " seconds\n");
    ...
    System.out.println(
        "\nverification lasts "
            + (double) (verifyEnd.getTime() - verifyBegin.getTime()) / 1000
            + " seconds\n");
    ...
    for (BigInteger j = BigInteger.valueOf(1);
        j.compareTo(m) < 1;
      ...
      for (BigInteger k = m.multiply(BigInteger.valueOf(-1));
          k.compareTo(m) < 1;
          k = k.add(BigInteger.valueOf(1))) {

        Point cp =
            applyDoubleAndAddMethod(
                basePoint, m.multiply(BigInteger.valueOf(2).multiply(k)), a, b, mod);
        ...
        if (cp.getPointX().compareTo(jP.getPointX()) == 0) {
          ...
          System.out.println(
              "order of group should be: "
                  + mod.add(BigInteger.valueOf(1))
                      .add(m.multiply(BigInteger.valueOf(2).multiply(k)))
                  + "±"
                  + j);

          BigInteger orderOfGroup =
              mod.add(BigInteger.valueOf(1)).add(m.multiply(BigInteger.valueOf(2).multiply(k)));
  ...
  public static Point pointAddition(Point P, Point Q, BigInteger a, BigInteger b, BigInteger mod)
      throws Exception {
    ...
    if (x1.compareTo(x2) == 0 && y1.compareTo(y2) == 0) {
      ...
      beta =
          (BigInteger.valueOf(3).multiply(x1.multiply(x1)).add(a))
              .multiply(multiplicativeInverse(BigInteger.valueOf(2).multiply(y1), mod));
    } else {

      // apply point addition

      beta = (y2.subtract(y1)).multiply(multiplicativeInverse(x2.subtract(x1), mod));
    }
  ...
  public static Point applyDoubleAndAddMethod(
      Point P, BigInteger k, BigInteger a, BigInteger b, BigInteger mod) throws Exception {
