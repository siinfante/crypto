...
public class Bitcoin {
  ...
  public static void main(String[] args) throws Exception {
    ...
    System.out.println(
        "base point: (" + basePoint.getPointX() + ", " + basePoint.getPointY() + ")\n");
    ...
    // BigInteger("11253563012059685825953619222107823549092147699031672238385790369351542642469");

    // BigInteger privateKey = new
    // BigInteger("18E14A7B6A307F426A94F8114701E7C8E774E7F9A47E2C2035DB29A206321725", 16);
    // BigInteger privateKey = new
    // BigInteger("945FB84E92575908A000D9D4A0B136EB0B69F8B5BCDAF157A3A91B7C83A02B1B", 16);
    BigInteger privateKey =
        new BigInteger("118CC1421845A67643244180F299147659E55C439B34E9E2380946579F64EF16", 16);

    System.out.println(
        "private key (hex): "
            + privateKey.toString(16)
            + " ("
            + privateKey.toString(16).length() * 4
            + " bits)\n");

    Point publicKey = EccOverFiniteField.applyDoubleAndAddMethod(basePoint, privateKey, a, b, mod);

    System.out.println(
        "public key: (" + publicKey.getPointX() + ", " + publicKey.getPointY() + ")");

    String publicKeyString =
        "04" + publicKey.getPointX().toString(16) + publicKey.getPointY().toString(16);
    ...
    System.out.println(
        "sha-256 applied hash: "
            + hash.toString(16)
            + " ("
            + hash.toString(16).length() * 4
            + " bits)");
    ...
    System.out.println(
        "ripemd160 applied hash: " + ripemdHashHex + " (" + ripemdHashHex.length() * 4 + " bits)");
    ...
    System.out.println(
        "adding network bytes to ripemd160 hash - extended ripemd160: " + ripemdHashHex + "\n");
    ...
    System.out.println(
        "sha-256 to extended ripemd160: "
            + new BigInteger(checking).toString(16)
            + " ("
            + new BigInteger(checking).toString(16).length() * 4
            + " bits)");
    ...
    System.out.println(
        "second time sha-256 applied to extended ripe160: "
            + checkingInt.toString(16)
            + " ("
            + checkingInt.toString(16).length() * 4
            + " bits)");
    ...
    System.out.println(
        "adding checksum to extended ripemd160 "
            + address
            + " ("
            + address.length() * 4
            + " bits)");
    ...
    System.out.println(
        "base 58 bitcoin address: " + base58Address + " (" + base58Address.length() * 4 + " bits)");
  }
